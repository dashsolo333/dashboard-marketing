...
export function renderBoard(ctx) {
  ...
  return h('div', { class: 'board' },
    doc.stages.map((stage) => {
      const items = ops.filter((o) => o.stageId === stage.id).sort((a, b) => a.priority.localeCompare(b.priority) || String(b.updatedAt).localeCompare(String(a.updatedAt)));
      const gate = stage.id === doc.gates.finalStageId ? 'GO requis' : stage.id === doc.gates.reviewStageId ? 'garde' : '';
      const col = h('section', {
