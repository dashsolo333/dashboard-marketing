// Checklist centrale d'un coup : synthèse, groupes par étape, statut et note par tâche.
import { h, icon, avatar, fmtDay, relTime, today } from './dom.js';
import { newId } from '../model/doc.js';
import { addChecklistItem, removeChecklistItem, updateChecklistItem, setChecklistStatus } from '../model/ops.js';
...
import { datePicker } from './opParts.js';

const statusOf = (id) => STATUSES.find((s) => s.id === id) || STATUSES[0];

export function renderChecklist(ctx, op, ro) {
  const doc = ctx.doc;
  const t = today();
  const st = checklistStats(op, t);
  const groups = groupItems(doc, op);
  let input; let groupSel;
  const add = () => {
    const text = input.value.trim();
    if (!text) { input.focus(); return; }
    input.value = ''; delete input.dataset.dirty;
    if (!ctx.act(`a ajouté une tâche à « ${op.title} »`, (d) => addChecklistItem(d, op.id, { id: newId('i'), text, group: groupSel.value, ...ctx.meta() }))) input.value = text;
    input.focus();
  };
  ...
  return h('section', { class: 'panel glass checklist-panel' },
    h('div', { class: 'checklist-head' },
      h('div', {},
        ...
        h('p', { class: 'checklist-sub' }, st.total ? `${st.done} faite${st.done > 1 ? 's' : ''} sur ${st.total}${st.total - st.done ? ` · ${st.total - st.done} restante${st.total - st.done > 1 ? 's' : ''}` : ' · tout est fait'}` : 'Ce qu’il reste à faire pour publier, étape par étape.')),
    ...
    st.total ? h('div', { class: 'check-stats' },
      ...
      stat('En cours', st.doing, '#8b5cf6'),
      stat('Bloquées', st.blocked, '#f87171', st.blocked > 0),
      stat('À faire', st.todo, '#8b8fa8'),
    ...
    groups.length ? h('div', { class: 'check-groups' }, groups.map((g) => h('section', { class: 'check-group', style: { '--gc': g.color } },
      ...
      h('ul', { class: 'checklist' }, g.items.map((it) => renderItem(ctx, op, it, { ro, t, groupOptions })))))) : null,

    ro ? null : h('div', { class: 'check-add' },
      h('span', { class: 'check-box is-ghost', 'aria-hidden': 'true' }),
      input = h('input', { class: 'input check-add-input', placeholder: 'Nouvelle tâche… puis Entrée', 'aria-label': 'Nouvelle tâche', dataset: { key: `add:${op.id}` }, onKeydown: (e) => { if (e.key === 'Enter') { e.preventDefault(); add(); } } }),
      groupSel = h('select', { class: 'select select-pill-sm', 'aria-label': 'Étape de la tâche' }, groupOptions.map((o) => h('option', { value: o.id, selected: o.id === op.stageId }, o.label))),
      h('button', { type: 'button', class: 'btn btn-cta btn-sm', onClick: add }, icon('plus'), 'Ajouter')),
    ro || st.total >= 12 ? null : h('div', { style: { marginTop: '10px' } },
      h('button', { type: 'button', class: 'btn btn-ghost btn-sm', onClick: () => ctx.act(`a ajouté la checklist type à « ${op.title} »`, (d) => applyTemplate(d, op.id, ctx.meta())) }, icon('check'), 'Insérer la checklist type (brief → publié)')));
}
...
function renderItem(ctx, op, it, { ro, t, groupOptions }) {
  const meta = () => ctx.meta();
  ...
  const saveText = () => {
    ...
    if (v && v !== it.text) ctx.act(`a modifié une tâche de « ${op.title} »`, (d) => updateChecklistItem(d, op.id, it.id, { text: v }, meta()));
    else textEl.value = it.text;
  };
  const setStatus = (s) => ctx.act(`a mis à jour une tâche de « ${op.title} »`, (d) => setChecklistStatus(d, op.id, it.id, s, meta()));
  const noteBox = h('div', { class: 'check-note', hidden: !noteOpen },
    noteEl = h('textarea', { class: 'textarea check-note-input', placeholder: 'Note : contexte, blocage, lien…', disabled: ro, rows: 2, dataset: { key: `note:${it.id}` },
      onChange: (e) => ctx.act(`a annoté une tâche de « ${op.title} »`, (d) => updateChecklistItem(d, op.id, it.id, { note: e.target.value.trim() }, meta())) }, it.note || ''));
  ...
  return h('li', { class: `check-item is-${it.status}${late ? ' is-late' : ''}` },
    ...
    h('div', { class: 'check-meta' },
      ...
      it.status !== 'done' && !ro ? datePicker(it.due, (v) => ctx.act(`a daté une tâche de « ${op.title} »`, (d) => updateChecklistItem(d, op.id, it.id, { due: v }, meta())), { placeholder: 'échéance' }) : null,
      ro ? null : h('select', { class: 'select select-pill-sm check-group-sel', 'aria-label': 'Étape', onChange: (e) => ctx.act(`a déplacé une tâche de « ${op.title} »`, (d) => updateChecklistItem(d, op.id, it.id, { group: e.target.value }, meta())) },
      ...
      ro ? null : h('button', { type: 'button', class: 'btn btn-ghost btn-sm btn-icon', 'aria-label': 'Retirer', onClick: () => { if (confirm(`Retirer la tâche « ${it.text} » ?`)) ctx.act(`a retiré une tâche de « ${op.title} »`, (d) => removeChecklistItem(d, op.id, it.id, meta())); } }, icon('close'))));
}
