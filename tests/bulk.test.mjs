...
const two = () => {
  let d = createOp(emptyDoc(), { id: 'a', title: 'A', by: who, at: now });
...
test('bulkMove moves what it can and lists what the gate blocks', () => {
  ...
  assert.equal(opById(ok.doc, 'b').stageId, 'brief');
  const forced = bulkMove(two(), ['a'], 'published', { by: who, at: now, force: true });
  assert.deepEqual(forced.moved, ['a']);
});

test('bulkUpdate and bulkDelete count what they touched', () => {
  const u = bulkUpdate(two(), ['a', 'ghost'], { priority: 'p0' }, { by: who, at: now });
  assert.equal(u.updated, 1);
  assert.equal(opById(u.doc, 'a').priority, 'p0');
